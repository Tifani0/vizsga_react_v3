...
export default function ServicesNail() {
  const [professionals, setProfessionals] = useState([]);
  const [bookingTarget, setBookingTarget] = useState(null);

  useEffect(() => {
    fetch("http://localhost:3000/nailbuilders")
      .then((r) => r.json())
  ...
  return (
    <>
      <div className="page-header">
        <h1>Műkörmös szolgáltatások</h1>
        <p className="page-subtitle">Válassz műkörmöst és foglalj időpontot!</p>
      </div>
              ...
              {prof.services?.map((service) => (
                <div className="service-item" key={service.id}>
                  ...
                  <button className="btn-book-service"
                    onClick={() => setBookingTarget({ professional: { ...prof, type: "Műkörmös" }, service })}>
                    Foglalás
                  </button>
                </div>
              ))}
